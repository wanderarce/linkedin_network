    ...
    this.search = function(inputName, inputCompany, inputOffice) {
        ...
        sigInst.iterNodes(function(data) {
            ...
            if (insere) {
                c.push({
                    id: data.id,
                    name: data.label,
                    company: data.attr.company,
                    office: data.attr.office
                });
        ...
        if (1 < c.length)
            for (var d = 0, h = c.length; d < h; d++) {
                var html = '<a href="#' + c[d].name + '" onclick="nodeActive(\'' + c[d].id + "')\">" + c[d].name;
                if (c[d].company != undefined && c[d].company != '') {
                    html += " - " + c[d].company;
                }
                if (c[d].office != undefined && c[d].office != '') {
                    html += " - " + c[d].office;
                }
...
function showGroups(a) {
    a ? ($GP.intro.find("#showGroups").text("Hide groups"),
            $GP.bg.show(), $GP.bg2.hide(), $GP.showgroup = !0) :
        ($GP.intro.find("#showGroups").text("View Groups"),
            $GP.bg.hide(), $GP.bg2.show(), $GP.showgroup = !1)
}
...
function nodeActive(a) {
    ...
    var createList = function(c) {
        ...
        for (g in c) {
            ...
            // if (d.attr.company != undefined && d.attr.company != null) {
            //     comp = d.attr.company;
            // }
            a != g && e.push({
                id: g,
    ...
    if (f.attributes) {
        ...
        if (image_attribute) {
            ...
            $GP.info_name.html("<div><span onmouseover=\"sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex['" + b.id + '\'])" onmouseout="sigInst.refresh()">' + b.label + " - " + b.company + "</span></div>");
        }
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        for (var f = [], e = [], c = 0, g = b.length; c < g; c++) {
            ...
            //!0 == d.hidden &&
            var item = '<li class="membership"><a href="#' + d.label + '" onmouseover="sigInst._core.plotter.drawHoverNode(sigInst._core.graph.nodesIndex[\'' + d.id + "'])\" onclick=\"nodeActive('" + d.id + '\')" onmouseout="sigInst.refresh()">' + d.label + " - " + d.company + "</a></li>";
            (
                e.push(b[c]),
                d.hidden = !1,
                d.attr.lineWidth = !1,
